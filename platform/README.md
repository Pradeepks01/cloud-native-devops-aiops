# ecommerce Microservices — Deployment Guide

This guide walks through the full deployment of the ecommerce e-commerce application — from running it locally with Docker, to provisioning AWS infrastructure with Terraform, to running it on Kubernetes with a full CI/CD pipeline, GitOps, and observability stack.
...
From the `platform/ecommerce-microservices/` directory:
...
The Terraform configuration in `platform/Infrastructure/` provisions everything needed to run the application on EKS.
...
Each service has a `Dockerfile` in its directory under `platform/ecommerce-microservices/`. When deployed to Kubernetes, these become container images stored in ECR, referenced by the manifests in `gitops/k8s/`.
...
├── namespace.yml            # Creates ecommerce namespace
...
```bash
kubectl get pods -n ecommerce -l app=ecommerce-postgres
...
2. Click the latest **ecommerce CI Pipeline** run
...
Prometheus is installed by Terraform via `kube-prometheus-stack`. It scrapes metrics from the cluster and the ecommerce services.
...
Grafana is also installed by `kube-prometheus-stack`. It is pre-configured with Prometheus as a datasource and comes with a custom ecommerce dashboard automatically loaded.
...
The **ecommerce Microservices** dashboard includes:
...
| Pod CPU Usage | CPU per pod in the ecommerce namespace |
...
Logs appear in **CloudWatch → Log groups → /eks/ecommerce/pods**.
