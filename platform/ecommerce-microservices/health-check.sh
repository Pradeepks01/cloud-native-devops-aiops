...
# ecommerce Microservices Health Check
echo "🔍 Checking ecommerce Microservices Health..."
echo ""
...
if pg_isready -h localhost -p 5432 >/dev/null 2>&1; then
    PRODUCT_COUNT_DB=$(psql -U postgres -d ecommerce_auth -t -c "SELECT COUNT(*) FROM products;" 2>/dev/null | xargs)
